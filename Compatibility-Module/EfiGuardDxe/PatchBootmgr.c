	...
	)
{
	...
	if (!gHeadless)
	{
		// Clear the screen and paint it green
	...
	{
		// Print info
		PRINT_IF_NOT_HEADLESS(L"[ %S!ImgArchStartBootApplication ]\r\n", (OriginalFunctionBytes == gBootmgrImgArchStartBootApplicationBackup ? L"bootmgr" : L"bootmgfw"));
		PRINT_IF_NOT_HEADLESS(L"ImageBase: 0x%p\r\n", ImageBase);
		PRINT_IF_NOT_HEADLESS(L"ImageSize: %lx\r\n", ImageSize);
		PRINT_IF_NOT_HEADLESS(L"File type: %S\r\n", FileTypeToString(FileType));
		PRINT_IF_NOT_HEADLESS(L"EntryPoint: 0x%p\r\n", ((UINT8*)ImageBase + HEADER_FIELD(NtHeaders, AddressOfEntryPoint)));
		PRINT_IF_NOT_HEADLESS(L"AppEntry:\r\n");
		PRINT_IF_NOT_HEADLESS(L"  Signature: %a\r\n", AppEntry->Signature);
		PRINT_IF_NOT_HEADLESS(L"  Flags: %lx\r\n", AppEntry->Flags);
		PRINT_IF_NOT_HEADLESS(L"  GUID: %08x-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x\r\n",
...
CallOriginal:
	if (!gHeadless)
	{
		if (FileType == WinloadEfi || FileType == BootmgrEfi)
		{
			...
			SetConsoleTextColour(EFI_LIGHTGRAY, TRUE); // Use light gray for restore
		}
	}
...
Exit:
	...
	if (!gHeadless)
	{
		...
		{
			PRINT_IF_NOT_HEADLESS(L"Successfully patched %S!%S.\r\n", ShortFileName, FunctionName);
			RtlSleep(500);
		}
	}
	return EFI_SUCCESS;
