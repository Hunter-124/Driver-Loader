	...
	)
{
	...
	PRINT_IF_NOT_HEADLESS(L"== Disassembling .text to find %S!ImgpValidateImageHash ==\r\n", ShortName);
	...
	{
		PRINT_IF_NOT_HEADLESS(L"Failed to initialize disassembler engine.\r\n");
	...
	{
		PRINT_IF_NOT_HEADLESS(L"    Failed to find %S!ImgpValidateImageHash%S.\r\n",
	...
	// Print info
	PRINT_IF_NOT_HEADLESS(L"    Patched %S!ImgpValidateImageHash [RVA: 0x%X].\r\n",
	...
	)
{
		...
		++Address)
	{
		if (CompareMem(Address, ImgpFilterValidationFailureMessage.Buffer, ImgpFilterValidationFailureMessage.Length) == 0)
		{
			...
			PRINT_IF_NOT_HEADLESS(L"    Found load failure string at 0x%llx.\r\n", (UINTN)IntegrityFailureStringAddress);
			break;
		}
	}
	...
	{
		PRINT_IF_NOT_HEADLESS(L"    Failed to find load failure string.\r\n");
		return EFI_NOT_FOUND;
	}
	...
	PRINT_IF_NOT_HEADLESS(L"== Disassembling %a to find %S!ImgpFilterValidationFailure ==\r\n", SectionName, ShortName);
											...
											Context.Operands)) != ZYDIS_STATUS_NO_MORE_DATA)
	{
			...
			Context.Operands[1].mem.base == ZYDIS_REGISTER_RIP)
		{
				...
				FindFunctionStart(ImageBase, NtHeaders, (UINT8*)Context.InstructionAddress) != NULL)
			{
				...
				PRINT_IF_NOT_HEADLESS(L"    Found load instruction for load failure string at 0x%llx.\r\n", (UINTN)LeaIntegrityFailureAddress);
				break;
			}
		}
	...
	{
		PRINT_IF_NOT_HEADLESS(L"    Failed to find %S!ImgpFilterValidationFailure%S.\r\n",
	...
	// Print info
	PRINT_IF_NOT_HEADLESS(L"    Patched %S!ImgpFilterValidationFailure [RVA: 0x%X].\r\n\r\n",
	...
	*OslFwpKernelSetupPhase1Address = NULL;
												...
												Context.Operands)) != ZYDIS_STATUS_NO_MORE_DATA)
		{
				...
				Context.Instruction.mnemonic == ZYDIS_MNEMONIC_CALL)
			{
					...
					OperandAddress == (UINTN)BlBdStop)
				{
					...
					{
						PRINT_IF_NOT_HEADLESS(L"    Found OslFwpKernelSetupPhase1 at 0x%llX.\r\n\r\n", (UINTN)(*OslFwpKernelSetupPhase1Address));
						return EFI_SUCCESS;
					}
				}
			}
		...
		++Address)
	{
		if (CompareGuid((CONST GUID*)Address, &gEfiAcpi20TableGuid))
		{
			...
			PRINT_IF_NOT_HEADLESS(L"    Found EFI ACPI 2.0 GUID at 0x%llX.\r\n", (UINTN)PatternAddress);
			break;
		}
	}
	...
	{
		PRINT_IF_NOT_HEADLESS(L"    Failed to find EFI ACPI 2.0 GUID.\r\n");
		return EFI_NOT_FOUND;
	}
			...
			Context.Operands[1].mem.base == ZYDIS_REGISTER_RIP)
		{
				...
				FindFunctionStart(ImageBase, NtHeaders, (UINT8*)Context.InstructionAddress) != NULL)
			{
				...
				if (Check[0] == 0x49 && Check[1] == 0x8D && Check[2] == 0x53) // If no match, this is not EfipGetRsdt
				{
					...
					PRINT_IF_NOT_HEADLESS(L"    Found load instruction for EFI ACPI 2.0 GUID at 0x%llX.\r\n", (UINTN)LeaEfiAcpiTableGuidAddress);
					break;
				}
			}
		}
	...
	{
		PRINT_IF_NOT_HEADLESS(L"    Failed to find load instruction for EFI ACPI 2.0 GUID.\r\n");
		return EFI_NOT_FOUND;
	}
	...
	if (EfipGetRsdt == NULL)
	{
		PRINT_IF_NOT_HEADLESS(L"    Failed to find EfipGetRsdt.\r\n");
		return EFI_NOT_FOUND;
	}

	PRINT_IF_NOT_HEADLESS(L"    Found EfipGetRsdt at 0x%llX.\r\n", (UINTN)EfipGetRsdt);
	...
	{
		PRINT_IF_NOT_HEADLESS(L"    Failed to find a single 'call EfipGetRsdt' instruction.\r\n");
		return EFI_NOT_FOUND;
	}
	...
	)
{
	...
	if (!gHeadless)
	{
		PRINT_IF_NOT_HEADLESS(L"HookedOslFwpKernelSetupPhase1 at 0x%p.\r\n", (VOID*)HookedOslFwpKernelSetupPhase1Address);
	}
...
Exit:
	if (!gHeadless)
	{
		...
		{
			PRINT_IF_NOT_HEADLESS(L"Successfully patched winload!OslFwpKernelSetupPhase1.\r\n");
			RtlSleep(500);
		}
	}
