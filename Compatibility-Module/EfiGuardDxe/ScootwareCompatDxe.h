...
//
// ScootwareCompat driver protocol handle
//
...
//
// TRUE if console output should be suppressed
//
extern BOOLEAN gHeadless;
...
//
#define PRINT_IF_NOT_HEADLESS(Fmt, ...) \
	do { \
		if (!gHeadless) Print(Fmt, ##__VA_ARGS__); \
	} while (FALSE)
