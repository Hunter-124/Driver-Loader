	...
	)
{
	...
	if (IsVbsEnabled())
	{
		Printf(L"Error: VBS (Virtualization Based Security) is enabled and running on this system.\n"
			...
			"Either the ScootwareCompat DXE driver is not loaded, or it failed to disable VBS during boot.\n"
			"Not continuing.\n");
	...
	if (!NT_SUCCESS(Status))
	{
		Printf(L"The ScootwareCompat DXE driver is either not loaded in SETVARIABLE_HOOK mode, or it is malfunctioning.\n");
		return Status;
	}
